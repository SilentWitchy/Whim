...
import net.august.whim.block.custom.GojiBerryBushBlock;
...
public class ModBlockStateProvider extends BlockStateProvider {
    ...
    protected void registerStatesAndModels() {
        ...
        horizontalBlock(ModBlocks.BOOKS_ONE.get(), modLoc("block/books_one"),  modLoc("block/books_two"),  modLoc("block/books_three"));
    }
    ...
    private ConfiguredModel[] states(BlockState state, String modelName, String textureName) {
        ...
        models[0] = new ConfiguredModel(models().cross(modelName + state.getValue(GojiBerryBushBlock.AGE),
                ResourceLocation.fromNamespaceAndPath(Whim.MOD_ID, "block/" + textureName + state.getValue(GojiBerryBushBlock.AGE))).renderType("cutout"));
    ...
    private ConfiguredModel[] states(BlockState state, CropBlock block, String modelName, String textureName) {
        ...
        models[0] = new ConfiguredModel(models().crop(modelName + state.getValue(((RadishCropBlock) block).getAgeProperty()),
                ResourceLocation.fromNamespaceAndPath(Whim.MOD_ID, "block/" + textureName + state.getValue(((RadishCropBlock) block).getAgeProperty()))).renderType("cutout"));
    ...
    private void blockItem(DeferredBlock<?> deferredBlock) {
        simpleBlockItem(deferredBlock.get(), new ModelFile.UncheckedModelFile("whim:block/" + deferredBlock.getId().getPath()));
    }

    private void blockItem(DeferredBlock<?> deferredBlock, String appendix) {
        simpleBlockItem(deferredBlock.get(), new ModelFile.UncheckedModelFile("whim:block/" + deferredBlock.getId().getPath() + appendix));
    }
}

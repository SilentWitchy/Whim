...
public class ModItemModelProvider extends ItemModelProvider {
    ...
    static {
        //This generates all the .json files which allow armor to be trim-able with these items, and defines the color of the trim.
    ...
    protected void registerModels() {
        ...
        basicItem(ModItems.WITCHY_SMITHING_TEMPLATE.get());
    ...
    private void trimmedArmorItem(DeferredItem<ArmorItem> itemDeferredItem) {
        final String MOD_ID = Whim.MOD_ID; // Change this to your mod id
...
//Due to more weird hard coding, these allow the button, fence, and wall to work.
    public void wallItem(DeferredBlock<?> block, DeferredBlock<Block> baseBlock) {
        ...
        //This makes sure tools are visible in third person mode.
        private ItemModelBuilder handheldItem(DeferredItem<?> item) {
            return withExistingParent(item.getId().getPath(),
                    ResourceLocation.parse("item/handheld")).texture("layer0",
                    ResourceLocation.fromNamespaceAndPath(Whim.MOD_ID,"item/" + item.getId().getPath()));
    }
}
